...
description: Install or update Storybook setup and story-writing conventions for React, Next.js, and TypeScript UI repositories. Use when Codex needs to detect Storybook, propose setup, add .storybook configuration, create story scripts, or establish component story conventions.
---
...
2. Prefer the official Storybook initializer for missing setup:
   - `pnpm dlx storybook@latest init`
   - `npm create storybook@latest`
   - `yarn create storybook`
3. Require approval before running initializer commands, installing
   dependencies, or editing `.storybook`, package, lock, or story files.
4. If setup exists, preserve framework adapter and only add missing scripts or
   conventions.
5. Add package scripts when approved:
   - `storybook`: `storybook dev -p 6006`
   - `build-storybook`: `storybook build`
6. Copy or adapt assets only when manual setup is requested:
