...
선택된 티켓의 계획, 변경 요약, 위험 분석, 검증 결과는 대상 레포의
`.agent-runs/{ticketKey}-{timestamp}/`에 기록됩니다. 선택 전 티켓 목록은
기본적으로 run 디렉터리를 만들지 않고 conversation state와 active-run state에만
보관합니다. 사람용 진행 요약은 하나의 `agent-run-report.md` 안에 단계별
section으로 누적합니다. 작업이 중단되면 이미 완료된 단계와 해결되지 않은 승인
상태를 확인해 이어서 진행합니다.
